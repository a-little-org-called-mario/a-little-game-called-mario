...
func move(delta :float):
	match state:
		IDLE, COOLDOWN:
			var collision :KinematicCollision2D= move_and_collide(Vector2.DOWN * ASCEND_SPEED * delta)
			if collision and collision.collider is Player and collision.position.y > (position.y+26):
...
func shoot_fireballs():
	if not (state == IDLE or state == CHARGING): return
	state= KILLING
	if player == null:
		player= self
...
func spawn_fireball(towards :Vector2= Vector2.ZERO):
	if towards == Vector2.ZERO: towards= position
...
func kill(killer):
	if (killer.position.x > position.x and $Sprite.flip_h) or (killer.position.x < position.x and !$Sprite.flip_h):
		turn_around()
		$TurnTimer.start()
	elif !is_mortal:
		$Idle1.play()
